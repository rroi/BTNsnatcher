...
class User(object):
	...
	def _getInput(self):
		q = input("Would you like to download extra torrents? (y/n) ")
		if q.lower() == "y" or q.lower() == "yes":
			extra = input("How much more would you like to download? ")
			try:
				extra = int(extra)
				if extra > 1000:
	...
	def bonus(self):
		...
		while True:
			try:
				months = float(input("Number of months at the end of which you will have enough points for Master: "))
				if months >= x:
				...
				ch = input("Would you like to try with another number of months? (y/n)")
				if ch.lower() != "y":
					break
			except:
	...
	def snatch(self):
		time.sleep(1)
		input("Press return to begin the initialization process. ")
		if self.needed <= 0:
		...
		print()
		input("Press return to begin downloading the torrent files. ")
		try:
	...
	try:
		...
		while True:
			...
			if ch == "1":
				...
				print("*" * 50)
				print()
				ch2 = input("Would you like to snatch torrents as well? (y/n) ")
				if ch2.lower() == "y":
					user.snatch()
				else:
					print("All done! Have a nice day!")
